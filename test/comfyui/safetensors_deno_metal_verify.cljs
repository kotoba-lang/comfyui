...
(defn- fixture-bytes []
  (let [json (.stringify js/JSON
        ...
        output (js/Uint8Array. (+ 8 (.-byteLength header) 6))
        ...
        (.then
         (fn [request]
           (let [backend (dg/backend request)
                 ...
                 direct? (boolean
                          (resolve 'num.deno-gpu/upload-f16-as-f32-byte-view))
                 checkpoint (safe/open-file path)
                 tensor (safe/read-tensor checkpoint backend "half")]
             (.then (arr/->vec tensor)
                    (fn [values]
                      (arr/release! tensor)
                      (let [reader (safe/reader-stats checkpoint)
                            stats (dg/backend-stats backend)]
                        (when-not (and (= [1.0 -2.0 3.0] values)
                                       (if direct?
                                         (and (= {"F16" 1} (:direct-dtypes reader))
                                              (= 6 (:direct-bytes reader)))
                                         (and (= 1 (:decoded-uploads reader))
                                              (= 3 (:decoded-elements reader))))
                        ...
                        (println "OK F16 safetensors Metal reader"
